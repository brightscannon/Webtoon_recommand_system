...
>  (장소 : 스타벅스 가산디지털단지)
>  - Token화 된 댓글 EDA (TF-IDF의 변형 모델로 분석[좋아요,싫어요,베스트 등의 요소를 가중치로 계산한 값을 사용])
...
>>      - ▶ Issue : 구 웹툰의 경우 200x150보다 작은 이미지가 다량존재.. 어떻게 할 것인가? --> 강제로 늘리기(resize를 사용)
>  - N사 : 썸네일 이미지 추가 확보 및 
>  - Token화된 댓글 EDA 진행 및 유사도 도출시도
>  - 뉴럴넷 이미지 분석 연구중...
>    - 방법 1 : 웹툰ID값을 분류값으로 놓고 학습 --> 웹툰ID기준 클러스터링 --> 클러스터링으로 분류된 그룹에 Numbering --> 분석시 카테고리값 데이터로 사용
